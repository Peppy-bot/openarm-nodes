...
use peppygen::exposed_actions::openarm_gripper_actions::v1::move_gripper;
use tracing::error;
...
async fn run_control_loop(
    ...
    // POS_FORCE (enactic v2 gripper reference): command the target motor angle with an
    // absolute speed limit and a torque-current (grip-force) cap, then recv_all. No
    // trajectory: the motor's POS_FORCE mode profiles to the setpoint; the gripper's short
    // travel needs no host-side profiling.
