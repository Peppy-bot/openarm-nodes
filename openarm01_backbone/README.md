...
```sh
peppy node info openarm01_backbone:v1
```
...
It's waiting on the robot_initializer's `is_ready`. Check the sim actually loaded with `peppy node info openarm01_robot_initializer_<engine>:v1`.
...
The downstream arm or gripper is unhealthy. Check its log with `peppy node info openarm01_arm_<engine>:v1`.
