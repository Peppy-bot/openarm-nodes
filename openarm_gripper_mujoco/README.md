...
Drives one side of the OpenArm gripper (either hardware generation, selected by `hardware_version`) inside MuJoCo. It implements `openarm_gripper_sim_passthrough:v1`, exposing the resolved sim-internal command stream while consuming the shared gripper-state contract.
