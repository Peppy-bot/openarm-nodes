...
Drives one side of the OpenArm gripper (either hardware generation, selected by `hardware_version`) inside MuJoCo. It conforms to `openarm_gripper:v1`, the same contract the real gripper drivers implement, so backbone and the UI work with it unchanged.
