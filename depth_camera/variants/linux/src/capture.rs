...
use peppygen::emitted_topics::depth_stream;
...
// Worst-case memory at Z16 640x480: 4 × ~614 KB ≈ 2.4 MB.
...
/// Drain `frame_rx` and publish frames in order on the `depth_stream` topic.
///
...
fn spawn_emit(
    ...
    Handle::current().spawn(async move {
        while let Some((stamp, id, frame)) = frame_rx.recv().await {
            let header = depth_stream::MessageHeader { stamp, frame_id: id };
            if let Err(e) =
                depth_stream::emit(&runner, header, encoding.clone(), width, height, frame).await
            {
