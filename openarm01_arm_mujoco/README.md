# openarm01_arm_mujoco

Drives one side of the OpenArm V10 (7 joints) inside MuJoCo. It conforms to `openarm01_arm:v1`, the same interface the real arm driver implements, so backbone and the UI work with it unchanged.

It attaches to the MuJoCo world that `openarm01_robot_initializer_mujoco` owns, so that node has to be running first. Targets beyond a joint's physical range are clamped to the model's limits, so the arm always goes as far as it can and the result says so.
...
peppy node run openarm01_arm_mujoco:v1 arm_id=0 -i left_arm_inst --bind sim@sim
peppy node run openarm01_arm_mujoco:v1 arm_id=1 -i right_arm_inst --bind sim@sim
```
...
The sim isn't running or hasn't finished loading. Check it with `peppy node info openarm01_robot_initializer_mujoco:v1`.
