...
use crate::controls::{ExposureMode, WhiteBalanceMode};
...
fn main() -> Result<()> {
    ...
    NodeBuilder::new().run(|params: Parameters, node_runner| async move {
        ...
        info!("opening V4L2 device {device_path} {width}x{height}@{fps}fps (requesting MJPG)");
        ...
        let (capture_format, active_width, active_height) =
            negotiate_format(&cap_dev, CaptureFormat::Mjpeg, width, height)
        ...
        spawn_set_exposure(node_runner.clone(), ctrl_dev.clone());
        spawn_set_white_balance(node_runner.clone(), ctrl_dev.clone());
        spawn_set_gain(node_runner.clone(), ctrl_dev.clone());
        spawn_set_brightness(node_runner.clone(), ctrl_dev.clone());
        spawn_set_contrast(node_runner.clone(), ctrl_dev);
...
fn spawn_set_exposure(runner: Arc<NodeRunner>, dev: Arc<Device>) {
    tokio::spawn(async move {
        loop {
            let dev = dev.clone();
            let result = set_exposure::handle_next_request(&runner, move |req| {
                let response = match ExposureMode::try_from(req.data.mode.as_str()) {
                    Ok(mode) => controls::set_exposure(&dev, mode, req.data.value),
                    Err(msg) => set_exposure::Response::new(false, msg, req.data.value),
                };
                Ok(response)
            })
            .await;
            if let Err(e) = result {
                error!("set_exposure: {e}");
            }
        }
    });
}

fn spawn_set_white_balance(runner: Arc<NodeRunner>, dev: Arc<Device>) {
    tokio::spawn(async move {
        loop {
            let dev = dev.clone();
            let result = set_white_balance::handle_next_request(&runner, move |req| {
                let response = match WhiteBalanceMode::try_from(req.data.mode.as_str()) {
                    Ok(mode) => controls::set_white_balance(&dev, mode, req.data.temperature),
                    Err(msg) => {
                        set_white_balance::Response::new(false, msg, req.data.temperature)
                    }
                };
                Ok(response)
            })
...
fn spawn_set_brightness(runner: Arc<NodeRunner>, dev: Arc<Device>) {
    tokio::spawn(async move {
        loop {
            let dev = dev.clone();
            let result = set_brightness::handle_next_request(&runner, move |req| {
                Ok(controls::set_brightness(&dev, req.data.value))
            })
            .await;
            if let Err(e) = result {
                error!("set_brightness: {e}");
            }
        }
    });
}

fn spawn_set_contrast(runner: Arc<NodeRunner>, dev: Arc<Device>) {
    tokio::spawn(async move {
        loop {
            let dev = dev.clone();
            let result = set_contrast::handle_next_request(&runner, move |req| {
                Ok(controls::set_contrast(&dev, req.data.value))
            })
            .await;
            if let Err(e) = result {
                error!("set_contrast: {e}");
            }
        }
    });
}
