...
//
// If telemetry hasn't started yet (the sim's first `joint_states_<side>`
// hasn't arrived), responds with all zeros. Backbone treats this the
// same way `is_ready` short-circuits: it polls until live values appear.

use std::sync::Arc;

use peppygen::NodeRunner;
use peppygen::exposed_services::get_joint_positions;
...
fn snapshot_positions(state: &Arc<SharedState>) -> [f64; DOF] {
    let guard = match state.joint_states.try_lock() {
        Ok(g) => g,
        Err(_) => {
            // Reader contention — return zeros rather than block the
            // service. Backbone will retry on its next poll cycle.
