        ...
        // Shutdown task: disables motors and releases lock on SIGINT/SIGTERM.
        {
            ...
            tokio::spawn(async move {
                ...
                info!("shutdown: disabling motors");
                {
                    ...
                    let mut arm = arm.lock().unwrap_or_else(|e| e.into_inner());
                    arm.disable_all();
            ...
            tokio::spawn(async move {
                loop {
                    if let Err(e) = get_joint_positions::handle_next_request(&runner, |_req| {
                        ...
                        // Zero-gain MIT command (τ=0 regardless of target) re-enables motors
                        // that timed out from inactivity and solicits a state response.
                        let zeros = [0.0f64; v10::ARM_DOF];
                        a.mit_control(&zeros, &zeros, &zeros, &zeros, &zeros);
                        a.recv_all(BRINGUP_RECV_US);
                        Ok(get_joint_positions::Response::new(a.get_state().positions.to_vec()))
                    })
                    .await
                    {
