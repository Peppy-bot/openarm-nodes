...
impl Pacer {
    ...
    pub async fn pace(&mut self) {
        ...
        if let Some((count, worst)) = self.due_report(Instant::now()) {
            warn!(
    ...
    /// The `(count, worst lateness)` to report, if any overruns accumulated and
    /// the report interval has elapsed; resets the tally. `None` otherwise.
    fn due_report(&mut self, now: Instant) -> Option<(u32, Duration)> {
        if self.overruns == 0 || now.duration_since(self.last_report) < OVERRUN_REPORT_PERIOD {
            return None;
        }
        let report = (self.overruns, self.worst_late);
        self.overruns = 0;
...
mod tests {
    ...
    fn due_report_only_after_interval_with_overruns() {
        ...
        assert_eq!(p.due_report(t0 + OVERRUN_REPORT_PERIOD), Some((1, PERIOD)));
        assert_eq!(p.overruns, 0);
